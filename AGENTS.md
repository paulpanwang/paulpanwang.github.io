...
- For the Open RSI Lab cards, use `Digital-World RSI` with `images/digital-world-rsi-cover.webp` and `Real-World RSI` with `images/real-world-rsi-cover.webp`. Do not use the older abstract covers `images/browser-agent-cover.png` or `images/rsi-real-world-agents-cover.png` for this section unless the user explicitly asks.
